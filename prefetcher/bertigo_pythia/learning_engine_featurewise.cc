...
// Returns which features would have chosen the same action as the combined decision.
// consensus[i] = true means Feature i agreed with the chosen action.
std::vector<bool> LearningEngineFeaturewise::getConsensusVec(State* state, uint32_t chosen_action)
{
  std::vector<bool> consensus(NumFeatureTypes, false);
  
  for (uint32_t index = 0; index < NumFeatureTypes; ++index) {
    if (m_feature_knowledges[index]) {
      uint32_t feature_action = m_feature_knowledges[index]->getMaxAction(state);
      consensus[index] = (feature_action == chosen_action);
    }
  }
